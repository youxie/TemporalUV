...
# fix generator and IUV discriminator, only train the image discriminator
...
if mode == 'train':
    ...
    model_path = "/mnt/netdisk1/youxie/Fashion_completion/test_0525/backup/model_0049.ckpt"
else:
    skip = 1
    recurrent_frames = 1
    is_training = False
...
if mode == 'train':
    ...
    if use_TEMP_GAN:
        loss += loss_uv_gradient * uv_gradient_factor * 0
    ...
    for step in range(1,max_iter):
        if step % decay_step == 0:
            ...
            factor_temp *= 1.0
            factor_img_l2 *= 1.0
            factor_uv_gradient *= 1.0
            factor_img_gradient *= 1.0
        if factor_l2 < 0.3:
            factor_l2 = 0.3
            factor_gradient = 0.3
            factor_uv = 0.3
