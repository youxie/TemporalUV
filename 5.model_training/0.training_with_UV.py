...
if mode == 'train':
    ...
    max_neighbor_y = 1
    data_mode = 1
    input_path = "/mnt/netdisk1/youxie/Fashion_extrapolation/IUV/" # original IUV
    IUV_path = "/mnt/netdisk1/youxie/Fashion_extrapolation_coherent_3/OF_texture_IUV_optimization_all_smooth_local_22_IUV/" # IUV after temporal relocation
    texture_reference = np.array(cv.imread("/mnt/netdisk1/youxie/Fashion_coordinate_2/data/ex_IUV_optimization_all_smooth/texture_0000_ex.png"),np.float32) # fixed texture t_o
    image_path = "/mnt/netdisk1/youxie/Fashion_extrapolation/example_rmbg_refined/" # RGB images
    model_path = ""
else:
    skip = 1
    recurrent_frames = 1
    ...
    texture_reference = np.array(cv.imread("texture_0000_ex.png"),np.float32)
...
if mode == 'train':
    for start_ds_x in range(data_mode):
        for start_ds_y in range(data_mode):
            ...
            for i in range(index_start,index_end,skip):
                IUV_ref_3 = np.load("%s%04d.npy"%(IUV_path,(i)))
...
if mode == "inference":
    ...
    for i in range(0,np.shape(input_IUV)[0]):
        ...
        if data_mode == 1:
            ...
            cv.imwrite(savepath + "gen_%04d.png"%(i+index_start), IUV_output)
            np.save(savepath + "gen_%04d.npy"%(i+index_start), IUV_output)
            ...
            image_with_t0 = TransferTexture_np(texture_reference/255.,np.zeros_like(image),IUV_output)
            image_grid_with_t0 = TransferTexture_np(texture_grid/255.,np.zeros_like(image),IUV_output)
            cv.imwrite(savepath + "/texture_%04d.png"%(i+index_start), texture_map)
            cv.imwrite(savepath + "/image_back_%04d.png"%(i+index_start), image_back)
            cv.imwrite(savepath + "/image_with_t0_%04d.png"%(i+index_start), image_with_t0)
            cv.imwrite(savepath + "/image_grid_with_t0_%04d.png"%(i+index_start), image_grid_with_t0)
        print("finish %dth frame! time cost:%f"%(i+index_start,time.time()-start_time))
...
if mode == 'train':
    ...
    if use_IMG_GAN:
        ...
        img_loss = tf.reduce_mean(tf.square(img_output-ref_img_tf_rh))

    loss = loss_l2 * l2_factor + loss_gradient * gradient_factor + loss_I
    if use_IMG_GAN:
    ...
    with tf.control_dependencies(update):
        if use_UV_GAN:
            ...
            loss = loss + gen_GAN_loss_UV * 10 * uv_factor
        if use_IMG_GAN:
        ...
        if use_TEMP_GAN:
            ...
            loss = loss + gen_GAN_loss_temp * 10 * temp_factor
    ...
    oldsaver = tf.train.Saver(max_to_keep=1,var_list=gen_var+disc_UV_var+disc_temp_var)

    sess.run(tf.initialize_all_variables())

    if model_path != "":
        oldsaver.restore(sess,model_path)
        print("model restored from %s"%model_path)
    ...
    factor_gradient = 1.0
    factor_uv = 1.0
    factor_img_gradient = 0.1
    factor_img = 0.1
    factor_temp = 0.1
    factor_img_l2 = 0.1
    factor_uv_gradient = 0.1
